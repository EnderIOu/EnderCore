...
public class EnchantXPBoost extends Enchantment implements IAdvancedEnchant {
    ...
    private static final EnumEnchantmentType ENCH_TYPE = EnumHelper.addEnchantmentType("EC_XPBOOST",
            new Predicate<Item>() {

                @Override
                public boolean apply(@Nullable Item item) {
                    return NullHelper.notnullM(item, "EnumEnchantmentType.canEnchantItem(null)").isDamageable() &&
                            !(item instanceof ItemArmor) && !(item instanceof ItemFishingRod);
                }
    ...
    private EnchantXPBoost() {
        ...
        super(Rarity.UNCOMMON, ENCH_TYPE,

 ...
 * <p>
 * Those can then be used to check if an item, item stack or block is in a list, or to fetch a list in item, item stack
 * or block form.
 * <p>
 * Non-existing things are silently ignored. Adding things in the pre-init phase (i.e. before all modded things exist in
 * the game) is safe.
 *
 */
public class Things extends Ingredient {
    ...
    /**
     * Returns a list of item stacks for this Thing. Wildcard values are expanded to a full list of all available
     * sub-items. Empty stacks are stripped.
     * 

...
public class ResourcePackAssembler {
    ...
    /**
     * @param directory
     *                  The directory to assemble the resource pack in. The name of the zip created will be the same as
     *                  this folder, and it will be created on the same
     *                  level as the folder. This folder will be <strong>WIPED</strong> on every call of
     *                  {@link #assemble()} .
     * @param packName
     ...
     * into the constructor on the same level as
     * that folder.
     *
     * @return The {@link ResourcePackAssembler} instance.
     */
    public ResourcePackAssembler assemble() {
        ...
        try {
            ...
            if (hasPackPng) {
                EnderFileUtils.copyFromJar(jarClass, modid + "/" + "pack.png",
                        new File(dir.getAbsolutePath() + "/pack.png"));
            }
     ...
     * same level as the directory passed into the
     * constructor.
     */
    public void inject() {
        if (FMLCommonHandler.instance().getEffectiveSide().isClient()) {
            try {
                if (defaultResourcePacks == null) {
                    defaultResourcePacks = ReflectionHelper.getPrivateValue(Minecraft.class, Minecraft.getMinecraft(),

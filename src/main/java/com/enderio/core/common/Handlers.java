...
public class Handlers {
    ...
    private static void registerHandler(Class<?> c, ASMData data, Handler handler) throws InstantiationException,
        ...
        for (Method m : methods) {
            if (m.isAnnotationPresent(SubscribeEvent.class)) {
                ...
                if (params.length >= 1) {
                    ...
                    l:
                    while (param != Event.class && param != null && param != Object.class) {
        ...
        EnderCore.logger.info(String.format("[Handlers] Registering handler %s to busses: %s", c.getSimpleName(),
                Arrays.deepToString(types.toArray())));
    ...
    private static Object tryInit(Handler annot, Class<?> c) {
        ...
        throw new RuntimeException(
                "Could not instantiate @Handler class " + c.getName() +
                        " or access INSTANCE field or instance() method or find static event handlers.");
    }
}

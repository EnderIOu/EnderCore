...
public class SmartTank extends FluidTank {
    ...
    public boolean canFillFluidType(@Nullable FluidStack resource) {
        return super.canFillFluidType(resource) && (restriction == null || (resource != null &&
                resource.getFluid() != null && FluidUtil.areFluidsTheSame(restriction, resource.getFluid())));
    }
    ...
    public int removeFluidAmount(int amount) {
        ...
        if (tile != null) {
            FluidEvent
                    .fireEvent(new FluidEvent.FluidDrainingEvent(fluid, tile.getWorld(), tile.getPos(), this, drained));
        }
        return drained;
    }
    ...
    public void writeCommon(@Nonnull String name, @Nonnull NBTTagCompound nbtRoot) {
        ...
        writeToNBT(tankRoot);
        if (restriction != null) {
            tankRoot.setString("FluidRestriction",
                    NullHelper.notnullF(restriction.getName(), "encountered fluid with null name"));
        }

...
public abstract class TileEntityBase extends TileEntity implements ITickable {
    ...
    public final void update() {
        ...
        if (/* !hasWorld() || isInvalid() || !world.isBlockLoaded(getPos()) || */ world.getTileEntity(getPos()) !=
                this || world.getBlockState(pos).getBlock() != getBlockType()) {
    ...
    /**
     * SERVER: Called when block data is sent (client receiving blocks from server, via notifyBlockUpdate). No need for
     * x/y/z tags.
     */
    @Override
    ...
    public void markDirty() {
        if (hasWorld() && world.isBlockLoaded(getPos())) { // we need the loaded check to make sure we don't trigger a
                                                           // chunk load while the chunk is loaded
    ...
    /**
     * Sends an update packet to all players who have this TileEntity loaded. Needed because inventory changes are not
     * synced in a timely manner unless the player

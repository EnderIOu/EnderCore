...
public abstract class ContainerEnderCap<T extends IItemHandler, S extends TileEntity> extends Container
    ...
    public @Nonnull ItemStack transferStackInSlot(@Nonnull EntityPlayer player, int fromSlotId) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (player.world.isRemote && ItemStack.areItemsEqual(slot.getStack(), itemstack)) {
                // it seems this slot depends on the server executing the move. Return a different value on client and
                // server to force a sync after the move is
    ...
    /**
     * Creates a mapping for shift-clicks from a slot ID (the on that was shift-clicked) to a list of {@link Slot}s (the
     * ones that can be inserted into).
     * <p>
     * Please note that the "try to fill up stacks" logic will look at <em>all</em> slots before the "move into empty
     * slot" logic runs.
     * 
     * @param fromSlotId
            ...
            for (Slot slot : targets) {
                if (isSlotEnabled(slot) && slot.getHasStack()) {
                    ...
                    if (stackInSlot.getItem() == stackToMove.getItem() &&
                            (!stackToMove.getHasSubtypes() ||
                                    stackToMove.getItemDamage() == stackInSlot.getItemDamage()) &&
                            ItemStack.areItemStackTagsEqual(stackToMove, stackInSlot) &&
                            slot.isItemValid(stackToMove) && stackToMove != stackInSlot) {

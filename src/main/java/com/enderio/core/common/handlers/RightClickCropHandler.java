    ...
    public static class LegacyPlantInfo implements IPlantInfo {
        ...
        public boolean init(@NotNull String source) {
            ...
            Block mcblock = ForgeRegistries.BLOCKS
                    .getValue(new ResourceLocation(NullHelper.notnullJ(blockinfo[0], "String.split()"),
                            NullHelper.notnullJ(blockinfo[1], "String.split()")));
            if (mcblock == null) {
                if (optional) {
                    return false;
                } else {
            ...
            for (IPlantInfo info : plants) {
                if (info.getGrownState() == blockState) {
                    if (event.getWorld().isRemote) {
                        ...
                        currentPlant = info;
                        blockState.getBlock().dropBlockAsItem(
                                NullHelper.notnullF(event.getWorld(), "RightClickBlock.getWorld()"), pos, blockState,
                                0);
                        currentPlant = null;
        ...
        if (currentPlant != null) {
            for (int i = 0; i < event.getDrops().size(); i++) {
                ...
                if (stack.getItem() == currentPlant.getSeed().getItem() &&
                        (currentPlant.getSeed().getItemDamage() == OreDictionary.WILDCARD_VALUE ||
                                stack.getItemDamage() == currentPlant.getSeed().getItemDamage())) {
                    stack.shrink(1);
                    if (stack.isEmpty()) {

    ...
    public void onPlayerTick(PlayerTickEvent event) {
        ...
        if (!player.world.isRemote && event.phase == Phase.END) {
            if (player.world.getTotalWorldTime() % 100 == 0) {
                ...
                if (cal.get(DAY_OF_MONTH) == 1 && cal.get(MONTH) == JANUARY && !player.getEntityData()
                        .getCompoundTag(EntityPlayer.PERSISTED_NBT_TAG).getBoolean("celebrated")) {
                    ...
                    player.sendMessage(
                            new TextComponentString(TextFormatting.AQUA + EnderCore.lang.localize("celebrate")));
                }
            }

...
public class XPBoostHandler {
    ...
    private static final Method getExperiencePoints = ReflectionHelper.findMethod(EntityLivingBase.class,
    ...
    public static void handleBlockBreak(BreakEvent event) {
        ...
        if (level >= 0) {
            ...
            if (xp > 0) {
                world.spawnEntity(new EntityXPOrb(world, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
                        getXPBoost(xp, level)));
            }
        }
    }

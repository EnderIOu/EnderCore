...
public class BottleFluidCapability implements IFluidHandlerItem, ICapabilityProvider {
    ...
    public int fill(FluidStack resource, boolean doFill) {
        if (container.getCount() != 1 || !isEmpty() || resource == null || resource.getFluid() != FluidRegistry.WATER ||
                resource.amount < ConfigHandler.waterBottleAmount) {
            return 0;
        } else {
            if (doFill) {
                fill();
    ...
    public FluidStack drain(FluidStack resource, boolean doDrain) {
        if (container.getCount() != 1 || !isFull() || resource == null || resource.getFluid() != FluidRegistry.WATER ||
                resource.amount < ConfigHandler.waterBottleAmount) {
            return null;
        } else {
            if (doDrain) {

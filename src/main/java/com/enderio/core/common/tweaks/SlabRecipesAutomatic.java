...
public class SlabRecipesAutomatic extends Tweak {
    ...
    SlabRecipesAutomatic() {
        super("slabToBlockRecipesAutomatic",
    ...
    private static void registerSlabToBlock() {
        ...
        for (IRecipe recipe : ForgeRegistries.RECIPES) {
            ...
            if ((outputItem.getItem() instanceof ItemSlab ||
                    Block.getBlockFromItem(outputItem.getItem()) instanceof BlockSlab) && outputItemName != null &&
                    ...
                    if (stack != null && !stack.isEmpty()) {
                        if (input == null) {
                            ...
                            if (input.getItem() == stack.getItem() && input.getItemDamage() == stack.getItemDamage()) {
                                ...
                                Log.info("Rejected anti-slab recipe for " + outputItem.copy() + " because " + input +
                                        " is not " + stack);
                                continue RECIPES;
                            }
                    }
                ...
                if (input != null && count == 3) {
                    ...
                    ForgeRegistries.RECIPES
                            .register(new ShapelessRecipes("", input, new NNList<>(ingredient, ingredient))
                                    .setRegistryName(outputItemName.getPath() + "_" + outputItem.getItemDamage() +
                                            "_to_" + (inputItemName != null ? inputItemName.getPath() : "no_idea")));
                    Log.info("Added anti-slab recipe for " + outputItem + " back to " + input);
                } else if (input != null) {
                    Log.info("Rejected anti-slab recipe for " + outputItem + " back to " + input +
                            " because count is " + count);
                }
            }

...
public class VecmathUtil {
    ...
    public static void computePlaneEquation(Vector4d a, Vector4d b, Vector4d c, Vector4d result) {
        computePlaneEquation(new Vector3d(a.x, a.y, a.z), new Vector3d(b.x, b.y, b.z), new Vector3d(c.x, c.y, c.z),
                result);
    }
     ...
     *         on the plane.
     */
    public static Vector3d computeIntersectionBetweenPlaneAndLine(Vector4d plane, Vector3d pointInLine,
     ...
     */
    public static void computeRayForPixel(Rectangle vp, Matrix4d ipm, Matrix4d ivm, int x, int y, Vector3d eyeOut,
                                          Vector3d normalOut) {
    ...
    public static Matrix4d createProjectionMatrixAsPerspective(double fovDegrees, double near, double far,
    ...
    public static Matrix4d createProjectionMatrixAsPerspective(double left, double right, double bottom, double top,
        ...
        Matrix4d matrix = new Matrix4d();
        matrix.set(new double[] { 2.0 * zNear / (right - left), 0.0, 0.0, 0.0, 0.0, 2.0 * zNear / (top - bottom), 0.0,
                0.0, A, B, C, -1.0, 0.0, 0.0, D, 0.0
    ...
    public static Matrix4d createProjectionMatrixAsOrtho(double left, double right, double bottom, double top,
    ...
    public static Matrix4d createMatrixAsLookAt(Vector3d eyePos, Vector3d lookAtPos, Vector3d upVec) {
        ...
        Matrix4d mat = new Matrix4d(sideVec.x, sideVec.y, sideVec.z, 0, upVed.x, upVed.y, upVed.z, 0, -forwardVec.x,
                -forwardVec.y, -forwardVec.z, 0, 0, 0, 0, 1);
    ...
    public static Vector3d preMultiply(Vector3d v, Matrix4d mat) {
        ...
        m.transpose();
        double d = 1.0f /
                (m.getElement(0, 3) * v.x + m.getElement(1, 3) * v.y + m.getElement(2, 3) * v.z + m.getElement(3, 3));
        double x = (m.getElement(0, 0) * v.x + m.getElement(1, 0) * v.y + m.getElement(2, 0) * v.z +
                m.getElement(3, 0)) * d;
        double y = (m.getElement(0, 1) * v.x + m.getElement(1, 1) * v.y + m.getElement(2, 1) * v.z +
                m.getElement(3, 1)) * d;
        double z = (m.getElement(0, 2) * v.x + m.getElement(1, 2) * v.y + m.getElement(2, 2) * v.z +
                m.getElement(3, 2) * d);
     ...
     *                   the look vector.
     */
    public static void getVectorsForMatrix(Matrix4d matrix, Vector3d upVecOut, Vector3d sideVecOut,
                                           Vector3d lookVecOut) {

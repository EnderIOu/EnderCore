...
public class ConfigHandler extends AbstractConfigHandler implements ITweakConfigHandler, IReloadCallback {
    ...
    @Config
    @Comment({ "Show durability on item tooltips.", "0 - Off", "1 - Always on", "2 - Only with shift",
            "3 - Only in debug mode" })
    @Range(min = 0, max = 3)
    @NoSync
    ...
    public boolean showInvisibleWarning() {
        ...
        boolean val = getValue("invisibilityWarning",
                "If set to true, the invisibility warning will show every time the user logs in.", false);
        saveConfigFile();
        return ret ? ret : val;
    }
    ...
    public void loadRightClickCrops() {
        JsonConfigReader<LegacyPlantInfo> reader = new JsonConfigReader<LegacyPlantInfo>(
                new ModToken(EnderCore.class, EnderCore.DOMAIN + "/config"),

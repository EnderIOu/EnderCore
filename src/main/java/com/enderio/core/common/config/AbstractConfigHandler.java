...
public abstract class AbstractConfigHandler implements IConfigHandler {
    ...
    static void addCommentDetails(Property prop, Bound<?> bound) {
        ...
        if (bound.equals(Bound.MAX_BOUND)) {
            prop.setComment(prop.getComment() + fmlLang.localize("default",
            ...
            prop.setComment(
                    prop.getComment() + fmlLang.localize("defaultNumeric", minIsInt ? bound.min.intValue() : bound.min,
     ...
     *         reload events. If this returns false, these

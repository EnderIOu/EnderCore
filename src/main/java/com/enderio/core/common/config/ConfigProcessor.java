 ...
 * fields to clients. It will also
...
public class ConfigProcessor {
     ...
     *                 </ul>
     * @param <ACTUAL>
     *                 The actual type of this adapter. This is what the field type must be for this adapter to be
     *                 applied.
     */
     ...
     * not pass a handler that has not been
     * {@link AbstractConfigHandler#initialize(File) initialized}!
     *
     * @param configs
     *                The class which contains your {@link Config} annotations (typically same class as your
     ...
     *
     * @param load
     *             If true, the values from the file will be loaded. Otherwise, the values existing in memory will be
     *             used.
     */
            ...
            switch (adapter.getType()) {
                ...
                case INTEGER:
                    if (defVal.getClass().isArray()) {
                        ...
                        res = AbstractConfigHandler.boundIntArr(prop,
                        ...
                        res = AbstractConfigHandler.boundValue(prop,
                                Bound.of(bound.min.intValue(), bound.max.intValue()), (Integer) defVal);
                    }
                    break;
                case STRING:
                    if (defVal.getClass().isArray()) {
            ...
            try {
                ...
                if (annot != null && !getNoSync(f)) {
                    ...
                    if (!oldVal.equals(newVal)) {
                        EnderCore.logger.debug("Config {}.{} differs from new data. Changing from {} to {}",
                                configs.getName(), f.getName(), oldVal, newVal);
                        f.set(null, newVal);
                        changed = true;
                    }
                    ...
                    EnderCore.logger.debug("Skipping syncing field {}.{} as it was marked NoSync", configs.getName(),
                            f.getName());
                }
            } catch (Exception e) {
                throw new RuntimeException(e);

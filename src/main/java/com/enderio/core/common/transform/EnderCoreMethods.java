    ...
    public static @NotNull ItemStack transferStackInSlot(@NotNull ContainerFurnace inv, @NotNull EntityPlayer playerIn,
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (index == 2) {
                ...
                if (TileEntityFurnace.isItemFuel(itemstack1) && mergeItemStack(inv, itemstack1, 1, 2, false)) { // HL:
                                                                                                                // added
                                                                                                                // this
                                                                                                                // case
                    // NOP - if we can move an item into the fuel slot, we're happy and done. Otherwise try to move it
                    // into the input slot, then stop moving it.
        ...
        if (!stack.isEmpty()) {
            if (stack.getItem() instanceof com.enderio.core.common.interfaces.IOverlayRenderAware) {
                ((com.enderio.core.common.interfaces.IOverlayRenderAware) stack.getItem())
                        .renderItemOverlayIntoGUI(stack, xPosition, yPosition);
            }
        ...
        if (!stack.isEmpty()) {
            if (stack.getItem() instanceof com.enderio.core.common.interfaces.IUnderlayRenderAware) {
                ((com.enderio.core.common.interfaces.IUnderlayRenderAware) stack.getItem())
                        .renderItemAndEffectIntoGUI(stack, xPosition, yPosition);
            }
    ...
    public static void processInitialInteract(EntityPlayer player, EnumHand hand) {
        if (!player.world.isRemote && !player.capabilities.isCreativeMode) {
            ...
            if (itemstack.getItem() instanceof INotDestroyedInItemFrames) {
                // The calling code got stored getHeldItem() before calling us and will check that stack when
                // determining if something happened to it

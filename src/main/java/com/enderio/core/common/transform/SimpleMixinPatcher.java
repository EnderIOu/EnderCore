    ...
    public byte[] transform(String name, String transformedName, byte[] targetClass) {
        ...
        if (!interfaceTargets.isEmpty()) {
            ...
            if (patches.size() > patchCount) {
                ...
                mixinLogger.info("Found {} {} to apply to class {} from implemented interfaces: {}", count,
                        count > 1 ? "patches" : "patch", transformedName,
                        targetNode.interfaces.stream().filter(interfaceTargets::containsKey).toArray());
            }
        }
        ...
        if (!patches.isEmpty()) {
            ...
            if (targetNode.visibleAnnotations != null && (targetNode.visibleAnnotations.stream()
                    .anyMatch(n -> n.desc.contains("com/enderio/core/common/transform/SimpleMixin") ||
                            n.desc.contains("com/enderio/core/common/mixin/SimpleMixin")))) {
            ...
            mixinLogger.info("Patching {} mixins onto class {} ({})", patches.size(), transformedName,
                    patches.stream().map(data -> data.source.replaceFirst("^.*\\.", ""))
            ...
            for (MixinData data : patches) {
                ...
                if (sourceClass == null) {
                    ...
                    if (!newInterfaces.isEmpty()) {
                        ...
                        mixinLogger.info("Added {} new {}: {}", newInterfaces.size(),
                                newInterfaces.size() == 1 ? "interface" : "interfaces", newInterfaces);
                    }
                    ...
                    if (!newMethods.isEmpty()) {
                        ...
                        mixinLogger.info("Added {} new {}: {}", newMethods.size(),
                                newMethods.size() == 1 ? "method" : "methods",
                                newMethods.stream().map(m -> m.name + m.desc).toArray());
                    }
                }
            }

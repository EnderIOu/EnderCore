        ...
        if (!par1ItemStack.isEmpty()) {
            ...
            while (!reversOrder && checkIndex < toIndex || reversOrder && checkIndex >= fromIndex) {
                ...
                if (isSlotEnabled(slot) && itemstack1.isEmpty() && slot.isItemValid(par1ItemStack)) {
                    ...
                    result = in.getCount() > 0; // Sanity check for slots which have a 0-size limit, if this stack count
                                                // is zero then no items were inserted and we should
                                                // return false.
                    break;
                }

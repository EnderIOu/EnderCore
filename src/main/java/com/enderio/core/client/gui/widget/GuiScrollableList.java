        ...
        for (int i = 0; i < getNumElements(); ++i) {
            ...
            if (elementY <= maxY && elementY + drawHeight >= minY) {
                ...
                if (showSelectionBox && i == selectedIndex) {
                    ...
                    renderer.pos(minX, elementY + drawHeight + 2, 0.0D).tex(0.0D, 1.0D)
                            .color(col.x, col.y, col.z, col.w).endVertex();
                    renderer.pos(maxX, elementY + drawHeight + 2, 0.0D).tex(1.0D, 1.0D)
                            .color(col.x, col.y, col.z, col.w).endVertex();
                    renderer.pos(maxX, elementY - 2, 0.0D).tex(1.0D, 0.0D).color(col.x, col.y, col.z, col.w)
                            .endVertex();
                    renderer.pos(minX, elementY - 2, 0.0D).tex(0.0D, 0.0D).color(col.x, col.y, col.z, col.w)
                            .endVertex();
                    renderer.putColor4(0);
                    renderer.pos(minX + 1, elementY + drawHeight + 1, 0.0D).tex(0.0D, 1.0D)
                            .color(colBlack.x, colBlack.y, colBlack.z, colBlack.w).endVertex();
                    renderer.pos(maxX - 1, elementY + drawHeight + 1, 0.0D).tex(1.0D, 1.0D)
                            .color(colBlack.x, colBlack.y, colBlack.z, colBlack.w).endVertex();
                    renderer.pos(maxX - 1, elementY - 1, 0.0D).tex(1.0D, 0.0D)
                            .color(colBlack.x, colBlack.y, colBlack.z, colBlack.w).endVertex();
                    renderer.pos(minX + 1, elementY - 1, 0.0D).tex(0.0D, 0.0D)
                            .color(colBlack.x, colBlack.y, colBlack.z, colBlack.w).endVertex();

...
public class VScrollbar implements IHideable {
    ...
    public void adjustPosition() {
        ...
        btnDown = new Rectangle(x, y + Math.max(0, height - EnderWidget.DOWN_ARROW_OFF.height),
                EnderWidget.DOWN_ARROW_OFF.width,
                EnderWidget.DOWN_ARROW_OFF.height);
        thumbArea = new Rectangle(x, y + btnUp.height, EnderWidget.VSCROLL_THUMB_OFF.width,
                Math.max(0, height - (btnUp.height + btnDown.height)));
    }
    ...
    public void drawScrollbar(int mouseX, int mouseY) {
        if (visible) {
            ...
            if (getScrollMax() > 0) {
                ...
                boolean hoverThumb = thumbArea.contains(mouseX, mouseY) && mouseY >= thumbPos &&
                        mouseY < thumbPos + EnderWidget.VSCROLL_THUMB_OFF.height;

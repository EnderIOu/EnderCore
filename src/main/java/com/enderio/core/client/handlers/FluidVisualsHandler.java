...
public class FluidVisualsHandler {
    ...
    private static final Field FfogColor1 = ReflectionHelper.findField(EntityRenderer.class, "fogColor1",
            "field_78539_ae");
    private static final Field FfogColor2 = ReflectionHelper.findField(EntityRenderer.class, "fogColor2",
            "field_78535_ad");
    private static final Field FbossColorModifier = ReflectionHelper.findField(EntityRenderer.class,
            "bossColorModifier", "field_82831_U");
    private static final Field FbossColorModifierPrev = ReflectionHelper.findField(EntityRenderer.class,
            "bossColorModifierPrev", "field_82832_V");
    private static final Field FcloudFog = ReflectionHelper.findField(EntityRenderer.class, "cloudFog",
            "field_78500_U");
        ...
        if (event.getState().getBlock() instanceof BlockFluidEnder) {
            ...
            if (entity instanceof EntityLivingBase) {
                if (((EntityLivingBase) entity).isPotionActive(MobEffects.WATER_BREATHING)) {
                    ...
                    event.setDensity(
                            0.1F - EnchantmentHelper.getRespirationModifier((EntityLivingBase) entity) * 0.03F);
                }
            } else {
    ...
    public static void onFogColor(EntityViewRenderEvent.FogColors event) throws IllegalArgumentException,
        ...
        if (event.getState().getBlock() instanceof BlockFluidEnder) {
            ...
            // the following was copied as-is from net.minecraft.client.renderer.EntityRenderer.updateFogColor() because
            // that %&!$ Forge event is fired after the
            ...
            if (entity instanceof EntityLivingBase &&
                    ((EntityLivingBase) entity).isPotionActive(MobEffects.BLINDNESS)) {
                int i = NullHelper
                        .notnull(((EntityLivingBase) entity).getActivePotionEffect(MobEffects.BLINDNESS), "LOC9903481")
                        .getDuration();
            ...
            if (entity instanceof EntityLivingBase &&
                    ((EntityLivingBase) entity).isPotionActive(MobEffects.NIGHT_VISION)) {
